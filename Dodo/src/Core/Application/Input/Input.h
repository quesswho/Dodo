...
namespace Dodo {
    ...
    public:
        ...
        // Keyboard
        void SetKeyState(int key, bool pressed) { if (IsValidKey(key)) m_Keys[key] = pressed; }
        bool IsKeyPressed(int key) const { return IsValidKey(key) && m_Keys[key]; }
        
        // Mouse
        void SetMouseState(int button, bool pressed) { if (IsValidMouse(button)) m_Mouse[button] = pressed; }
