...
namespace Dodo::Math {
    ...
    struct Mat3x3 {
        ...
        Mat3x3(const Math::TVec3<T>& first, const Math::TVec3<T>& second, const Math::TVec3<T>& third)
        {
        ...
        const Mat3x3& operator+=(const float scalar)
        {
            for (int i = 0; i < 9; i++) {
        ...
        static const Mat3x3 Inverse(const Mat3x3& mat)
        {
        ...
        static const Mat3x3 Rotate(const float radians)
        {
            Mat3x3 result;
