...
namespace Dodo { namespace Platform {
    ...
    Ref<Shader> OpenGLShaderBuilder::BuildVertexFragmentShader(const ShaderBuilderFlags flags, const char *name) const
    {
        ...
        if (flags & ShaderBuilderFlagNormalMap)
            fragment.append(
                "	vec3 normal = normalize(texture(u_NormalMap, frag_in.TexCoord.xy).rgb * 2.0f - 1.0f);\n");
        else if (!(flags & ShaderBuilderFlagCubeMap))
            ...
            flags & ShaderBuilderFlagNormalMap)
        {
            if (flags & ShaderBuilderFlagNormalMap)
            {
                ...
                if (flags & ShaderBuilderFlagSpecularUniform)
                    fragment.append(
                        "	vec3 specular = pow(max(dot(normal, halfwayDir), 0.0f), 32.0f) * vec3(u_Specular);\n");
