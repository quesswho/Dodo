...
{
    if (!chunk->m_Vertbuffer) return;

    Dodo::Math::Mat4 model = Dodo::Math::Mat4::Translate(
        Dodo::Math::Vec3((chunk->m_ChunkPos.x << 4), (chunk->m_ChunkPos.z << 4), (chunk->m_ChunkPos.y << 4)));
